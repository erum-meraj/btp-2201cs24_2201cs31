...
class WorkflowMemory:
    ...
    def extract_workflow_features(self, workflow_dict: dict) -> dict:
        ...
        nodes_in1_out1 = sum(
            1 for i in range(1, N + 1) if in_degree[i] == 1 and out_degree[i] == 1
        )
    ...
    def extract_environment_features(self, env_dict: dict) -> dict:
        ...
        dr_values = [
            v for (src, dst), v in DR_map.items() if src != dst and v < float("inf")
        ]
    ...
    def generate_textual_summary(
        self, workflow_features: dict, env_features: dict, params: dict
    ) -> str:
        """
        ...
        summary_lines.append(
            f"  - Avg compute: {wf['v_mean']:.2e} cycles (std: {wf['v_std']:.2e})"
        )
        summary_lines.append(
            f"  - Edge density: {wf['edge_density']:.2%} ({wf['edge_count']} edges)"
        )
        summary_lines.append(f"  - Avg data dependency: {wf['d_mean']:.2e} bytes")
        summary_lines.append(
            f"  - Critical path length: ~{wf['critical_path_length']} hops"
        )
        ...
        summary_lines.append(
            f"  - {env['edge_count']} edge, {env['cloud_count']} cloud servers"
        )
    ...
    def save_execution(
        ...
        # Save to file
        filename = (
            f"memory_{experiment_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def compute_feature_vector(
        self, wf_features: dict, env_features: dict
    ) -> np.ndarray:
        """
        ...
        features = [
            ...
            wf_features.get("critical_path_length", 0)
            / max(wf_features.get("N", 1), 1),  # Normalize
    ...
    def retrieve_similar_executions(
        self, workflow_dict: dict, env_dict: dict, params: dict, top_k: int = 3
    ) -> List[dict]:
        """
        ...
        current_vector = self.compute_feature_vector(
            current_wf_features, current_env_features
        )
        ...
        for filepath in memory_files:
            try:
                ...
                if memory_record.get("params", {}).get("delta_t") != params.get(
                    "delta_t"
                ) or memory_record.get("params", {}).get("delta_e") != params.get(
                    "delta_e"
                ):
                    continue
