...
from agents.evaluator_agent.candidate_generator.candidate_generator import CandidatePolicyGenerator
...
class EvaluatorAgent:
    ...
    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result["best_policy"] is None:
            evaluation = (f"No finite-cost policy found. "
    ...
    def find_best_policy(
        ...
        if self.weak_solver.is_enabled() and result['best_policy']:
            ...
            if refined['cost'] < result['best_cost']:
                print(f"  ✓ Weak solver improved cost: "
    ...
    def _get_llm_candidates(
        ...
        prompt_file_path = os.path.join(os.path.dirname(__file__), 'evaluator_prompt.md')
    ...
    def _create_inline_prompt(self, env_details, num_tasks, task_details, params, plan, location_ids):
...
"""
    ...
    def _format_task_details(self, tasks: Dict, edges) -> str:
        ...
        for task_id in sorted(tasks.keys()):
            ...
            children_deps = [(k, edges_dict[(int(task_id), k)]) 
                           for (j, k), _ in edges_dict.items() if j == int(task_id)]
    ...
    def _format_env_for_prompt(self, env_dict: Dict[str, Any]) -> str:
        ...
        if 'DR' in env_dict:
            details.append(f"Data transfer rates available: {len(env_dict['DR'])} pairs")
        if 'VR' in env_dict:
    ...
    def _print_problem_info(
        ...
        print(f"  Cost Model: U(w,p) = {params.get('delta_t', 1)}*T + "
