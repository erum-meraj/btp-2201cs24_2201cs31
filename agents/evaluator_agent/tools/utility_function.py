...
class UtilityFunctionTool:
    ...
    def evaluate_batch(
        self, candidates: list, workflow, environment, show_progress: bool = True
        ...
        for idx, policy in enumerate(candidates, 1):
            ...
            if show_progress and (idx % 100 == 0 or idx == total):
                finite_count = sum(
                    1 for r in results.values() if not math.isinf(r["total"])
                )
                print(
                    f"  Progress: {idx}/{total} policies evaluated "
    ...
    def find_best_policy(
        self, candidates: list, workflow, environment
    ) -> Dict[str, Any]:
        """
    ...
    def _print_breakdown(
        self, policy: Tuple[int, ...], result: Dict[str, float]
    ) -> None:
