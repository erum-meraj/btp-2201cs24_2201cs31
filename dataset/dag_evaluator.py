...
class UtilityEvaluator:
    ...
    def __init__(
        self, CT: float = 0.2, CE: float = 1.2, delta_t: int = 1, delta_e: int = 1
    ):
        self.CT = CT
        self.CE = CE
    ...
    def compute_ED(
        self, workflow: Workflow, placement: Dict[int, int], env: Environment
    ) -> float:
    ...
    def compute_critical_path_delay(
        self, workflow: Workflow, placement: Dict[int, int], env: Environment
    ) -> float:
        ...
        for u in order:
            for v in Ki.get(u, []):
                edge_weight = self.compute_delay_edge_weight(
                    u, v, workflow, placement, env
                )
...
def find_optimal_policy(
    ...
    for placement_tuple in itertools.product(location_ids, repeat=N):
        ...
        if (
            ...
            and checked % max(1, total_combinations // 20) == 0
        ):
            progress = 100 * checked / total_combinations
            print(
                f"    Progress: {checked:,}/{total_combinations:,} ({progress:.1f}%) - Best: {best_cost:.2f}, Valid: {valid_found}"
            )
...
# ==================== MAIN PROCESSING ====================
def process_dataset(
    json_file_path: str, output_file_path: str, start_int: int, end_ind: int
):
    ...
    # Summary statistics
    valid_costs = [
        r["optimal_cost"] for r in results if not math.isinf(r["optimal_cost"])
    ]
    if valid_costs:
        print(f"\nSummary Statistics:")
